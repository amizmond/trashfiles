...
public class UploadMasterService : IUploadMasterService
{
    ...
    public async Task UploadAsync(Stream fileStream)
    {
        ...
        var programsByName = BuildNameLookup(programs, p => p.Name);
        ...
        var portfolioEpicsByName = BuildNameLookup(
            portfolioEpics.Where(pe => !string.IsNullOrWhiteSpace(pe.Name)), pe => pe.Name!);
        ...
        var boByName = BuildNameLookup(
            businessOutcomes.Where(bo => !string.IsNullOrWhiteSpace(bo.Name)), bo => bo.Name!);
        ...
        foreach (var row in rows)
        {
            ...
            if (!string.IsNullOrWhiteSpace(row.Program))
            {
                ...
                if (program == null)
                {
                    program = new StrategicObjective { Name = name };
            ...
            if (!string.IsNullOrWhiteSpace(row.BoParent) || !string.IsNullOrWhiteSpace(row.BoParentId))
            {
                ...
                {
                    portfolioEpic = new PortfolioEpic { Name = epicName, JiraId = epicJiraId };
