...
public class FeatureUploadServiceTests
{
    ...
    private Task SeedProjectAsync() =>
        _db.SeedAsync(db => db.CapitalProjects.Add(new Art { Id = 50, Name = "Atlas", JiraKeys = [new ArtJiraKey { JiraKey = "ATL" }] }));
    ...
    [Fact]
    public async Task Any_of_an_arts_project_keys_is_accepted()
    {
        await _db.SeedAsync(db => db.CapitalProjects.Add(new Art
        {
            Id = 50,
            Name = "Atlas",
            JiraKeys = [new ArtJiraKey { JiraKey = "ATL" }, new ArtJiraKey { JiraKey = "ORB" }]
        }));

        var result = await ParseAsync(Workbook(CoreHeaders, new[] { "ORB", "ORB-1", "Login", "Build it" }));
