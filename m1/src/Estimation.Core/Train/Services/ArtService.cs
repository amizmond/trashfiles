...
public partial class ArtService : IArtService
{
    ...
    private static async Task UpdateKeyRowsAsync(EstimationDbContext context, Art existing, IReadOnlyList<ArtJiraKey> rows)
    {
        ...
        var affectedKeys = added.Select(r => r.JiraKey)
            .Concat(refiltered.Select(r => current[r.JiraKey].JiraKey))
            .Concat(removed.Select(k => k.JiraKey))
            .ToList();
        if (affectedKeys.Count > 0)
        {
            var arts = await ArtIdsOnKeysAsync(context, affectedKeys);
            arts.Add(existing.Id);
            await ResetSyncWatermarksAsync(context, arts);
        }
    ...
    private static async Task<HashSet<int>> ArtIdsOnKeysAsync(EstimationDbContext db, IReadOnlyCollection<string> jiraKeys)
    {
        var artIds = await db.CapitalProjectJiraKeys
            .Where(k => jiraKeys.Contains(k.JiraKey))
            .Select(k => k.CapitalProjectId)
            .Distinct()
            .ToListAsync();
        return artIds.ToHashSet();
    }

    private static async Task ResetSyncWatermarksAsync(EstimationDbContext db, IReadOnlyCollection<int> artIds)
    {
        if (artIds.Count == 0)
        {
            return;
        ...
        var settings = await db.JiraSyncProjectSettings
            .Where(s => artIds.Contains(s.CapitalProjectId))
            .ToListAsync();
        foreach (var s in settings)
        {
            s.LastSyncedWatermarkUtc = null;
        }
    }
    ...
    public async Task<bool> DeleteAsync(int id)
    {
        ...
        if (cp.JiraKeys.Count > 0)
        {
            var arts = await ArtIdsOnKeysAsync(db, cp.JiraKeys.Select(k => k.JiraKey).ToList());
            arts.Remove(cp.Id);
            await ResetSyncWatermarksAsync(db, arts);
        }
