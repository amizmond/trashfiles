...
public record JiraSyncItem(string JiraKey, string? Summary, string? Description, string? AcceptanceCriteria, string? NavigatorId, string? IssueType, string? Labels, string? Components, string? Status, DateTime? JiraUpdated, DateTime? TargetStart, DateTime? TargetEnd, int? StoryPoints)
    : IJiraScalarSyncItem
{
...
public class StrategicObjectiveService : IStrategicObjectiveService
{
    ...
    public async Task<JiraSyncResult> SyncFromJiraAsync(int capitalProjectId, string projectKey, List<JiraSyncItem> items)
    {
        ...
        var toLink = capitalProjectId > 0 ? perItem : new List<StrategicObjective>();
        foreach (var so in toLink)
        {
        ...
        Log.Information("Jira sync for Art {Id}: created={Created}, updated={Updated}, linked={Linked}",
