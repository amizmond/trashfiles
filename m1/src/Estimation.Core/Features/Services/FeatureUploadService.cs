...
public class FeatureUploadService : IFeatureUploadService
{
    ...
    {
        var projectKeys = await db.CapitalProjectJiraKeys
            .Select(k => k.JiraKey)
        ...
        var technicalApprovalValues = await db.TechnicalApprovals
            .OrderBy(ta => ta.SortOrder).ThenBy(ta => ta.Name)
            .Select(ta => ta.Name)
            .ToListAsync();
        ...
        return new FeatureExportLookups
        {
            ...
            TechnicalApprovalValues = technicalApprovalValues,
    ...
    public async Task<FeatureParseResult> ParseFileAsync(Stream fileStream, FeatureUploadColumnSelection selection)
    {
        ...
        var projectKeySet = new HashSet<string>(
            await db.CapitalProjectJiraKeys.Select(k => k.JiraKey).ToListAsync(),
            StringComparer.OrdinalIgnoreCase);
        ...
        var technicalApprovalIdByName = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
        string? defaultTechnicalApprovalName = null;
        foreach (var ta in await db.TechnicalApprovals.AsNoTracking().Select(t => new { t.Id, t.Name }).ToListAsync())
        {
            if (!string.IsNullOrWhiteSpace(ta.Name))
            {
                technicalApprovalIdByName.TryAdd(ta.Name.Trim(), ta.Id);
                if (ta.Id == TechnicalApproval.DefaultId)
                {
                    defaultTechnicalApprovalName = ta.Name.Trim();
                }
            }
        }
        ...
        foreach (var dataRow in dataRows)
        {
            ...
            row.CurrentTechnicalApproval = existing?.TechnicalApproval?.Name;
            if (Apply(FeatureUploadColumn.TechnicalApproval))
            {
                row.TechnicalApproval = Norm(GetCell(dataRow, colMap, FeatureUploadColumn.TechnicalApproval));
                if (!string.IsNullOrWhiteSpace(row.TechnicalApproval))
                {
                    if (technicalApprovalIdByName.TryGetValue(row.TechnicalApproval, out var taId))
                    {
                        row.TechnicalApprovalId = taId;
                    }
                    else
                    {
                        row.ValidationErrors[nameof(FeatureUploadRow.TechnicalApproval)] =
                            $"Design Approval '{row.TechnicalApproval}' not found";
                    }
                }
                else
                {
                    row.TechnicalApprovalId = TechnicalApproval.DefaultId;
                    row.TechnicalApproval = defaultTechnicalApprovalName;
                }
                row.TechnicalApprovalChanged = existing is not null
                    && existing.TechnicalApprovalId != row.TechnicalApprovalId;
            }
            ...
            if (Apply(FeatureUploadColumn.ExternalDependencies))
            {
                row.ExternalDependenciesRaw = Norm(GetCell(dataRow, colMap, FeatureUploadColumn.ExternalDependencies));
                row.ExternalDependencies = ParseBoolean(row.ExternalDependenciesRaw);
                row.CurrentExternalDependencies = existing?.ExternalDependencies;
                row.ExternalDependenciesChanged = existing is not null
                    && row.ExternalDependencies is not null
                    && existing.ExternalDependencies != row.ExternalDependencies;
            }
            else
            {
                row.CurrentExternalDependencies = existing?.ExternalDependencies;
    ...
    private static void ValidateRow(FeatureUploadRow row, HashSet<string> projectKeySet)
    {
        ...
        if (!string.IsNullOrWhiteSpace(row.ExternalDependenciesRaw) && row.ExternalDependencies is null)
        {
            row.ValidationErrors[nameof(FeatureUploadRow.ExternalDependencies)] =
                $"'{row.ExternalDependenciesRaw}' is not a valid Yes/No value";
        }
    ...
    private static bool? ParseBoolean(string? value)
    {
        ...
        var v = value.Trim();
        if (v.Equals(FeatureExcelExportService.BooleanYes, StringComparison.OrdinalIgnoreCase)
            || v.Equals("True", StringComparison.OrdinalIgnoreCase) || v == "1")
        {
            return true;
        }
        if (v.Equals(FeatureExcelExportService.BooleanNo, StringComparison.OrdinalIgnoreCase)
