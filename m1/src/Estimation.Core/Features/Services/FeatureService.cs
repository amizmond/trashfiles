...
public record JiraFeatureSyncItem(string JiraKey, string? Summary, string? Description, string? AcceptanceCriteria, string? NavigatorId, string? IssueType, string? Labels, string? Components, string? FeatureName, string? RagExplain, string? ParentLink, string? Status, DateTime? JiraUpdated, DateTime? TargetStart, DateTime? TargetEnd, int? StoryPoints, string? GfedTeam, string? PlanningIncrement)
    : IJiraScalarSyncItem
{
...
public interface IFeatureService
{
    ...
    Task<Feature> UpdateAsync(Feature feature, IssueMatchFacts? expectedArtFacts = null);
...
public static class FeatureArtFacts
{
    public const string ChangedMessage =
        "This feature's project, labels or components changed since you opened it. Reload the page before saving.";

    public static IssueMatchFacts Of(Feature feature) =>
        new(feature.ProjectKey, feature.JiraId, feature.Labels, feature.Components);

    public static bool Changed(IssueMatchFacts loaded, IssueMatchFacts stored) =>
        !SameKey(loaded.ProjectKey, stored.ProjectKey)
        || !SameKey(JiraProjectKeys.Of(loaded.ProjectKey, loaded.JiraId), JiraProjectKeys.Of(stored.ProjectKey, stored.JiraId))
        || !SameValues(loaded.Labels, stored.Labels)
        || !SameValues(loaded.Components, stored.Components);

    private static bool SameKey(string? a, string? b) =>
        string.Equals(JiraProjectKeys.Normalize(a), JiraProjectKeys.Normalize(b), StringComparison.Ordinal);

    private static bool SameValues(string? a, string? b) => ValuesOf(a).SetEquals(ValuesOf(b));

    private static HashSet<string> ValuesOf(string? csv) =>
        (csv ?? string.Empty)
...
public class FeatureService : IFeatureService
{
    ...
    public async Task<Feature> UpdateAsync(Feature feature, IssueMatchFacts? expectedArtFacts = null)
    {
        await using var db = await _ctx.CreateDbContextAsync();
        ...
        var stored = FeatureArtFacts.Of(existing);
        if (expectedArtFacts is not null
            && FeatureArtFacts.Changed(expectedArtFacts, stored)
            && FeatureArtFacts.Changed(FeatureArtFacts.Of(feature), stored))
        {
            Log.Warning("Feature {FeatureId} not saved: its project, labels or components changed since it was loaded", feature.Id);
            throw new InvalidOperationException(FeatureArtFacts.ChangedMessage);
        }
        ...
        existing.Components = feature.Components;
