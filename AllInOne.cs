...
public sealed class ExcelCellWriter : IExcelCellWriter
{
    ...
    private void WriteStringValue(OpenXmlWriter writer, string cellReference, string stringValue, uint? styleIndex)
    {
        ...
        writer.WriteElement(new CellValue(sharedStringId.ToString()));
        writer.WriteEndElement();
    }
...
public class ExcelColumnAttribute : Attribute
{
    ...
    public ExcelColumnAttribute(string name, int orderId)
        : this(name)
    {
        OrderId = orderId;
    }

    public ExcelColumnAttribute(string name, int orderId, string hexColor)
        : this(name, orderId)
    {
        Color = hexColor;
    }
...
/// <summary>
/// Interface for building Excel files with a fluent API
/// </summary>
public interface IExcelFileBuilder
{
    /// <summary>
    /// Adds a new sheet with configuration options
    /// </summary>
    /// <typeparam name="T">The type of data model for the sheet</typeparam>
    /// <param name="sheetName">Name of the sheet</param>
    /// <returns>Sheet builder for further configuration</returns>
    ISheetBuilder<T> AddSheet<T>(string sheetName) where T : class, new();

    /// <summary>
    /// Adds a simple sheet with data and minimal configuration
    /// </summary>
    /// <typeparam name="T">The type of data model for the sheet</typeparam>
    /// <param name="data">Data to be added to the sheet</param>
    /// <param name="sheetName">Optional sheet name</param>
    /// <returns>The file builder for method chaining</returns>
    IExcelFileBuilder AddSimpleSheet<T>(List<T> data, string? sheetName = null) where T : class;

    /// <summary>
    /// Configures file properties such as title, author, etc.
    /// </summary>
    /// <param name="configure">Action to configure file properties</param>
    /// <returns>The file builder for method chaining</returns>
    IExcelFileBuilder WithFileProperties(Action<ExcelFileProperties> configure);

    /// <summary>
    /// Builds the Excel file and returns it as a memory stream
    /// </summary>
    /// <param name="cancellationToken">Cancellation token for async operations</param>
    /// <returns>Memory stream containing the Excel file</returns>
    MemoryStream Build(CancellationToken cancellationToken = default);
}
...
public sealed class ExcelFileBuilder : IExcelFileBuilder
{
    ...
    private readonly ExcelFileProperties _fileProperties = new();
    ...
    public ISheetBuilder<T> AddSheet<T>(string sheetName) where T : class, new()
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(sheetName);
        return new SheetBuilder<T>(this, sheetName);
    }

    public IExcelFileBuilder AddSimpleSheet<T>(List<T> data, string? sheetName = null) where T : class
    {
        ArgumentNullException.ThrowIfNull(data);
    ...
    public IExcelFileBuilder WithFileProperties(Action<ExcelFileProperties> configure)
    {
        ArgumentNullException.ThrowIfNull(configure);
        configure(_fileProperties);
        return this;
    }
    ...
    private void BuildExcelDocument(MemoryStream memoryStream, CancellationToken cancellationToken)
    {
        ...
        // Initialize shared string table once for the entire workbook
        _cellWriter.InitializeSharedStringTable(workbookPart);
    ...
    private void AddDocumentProperties(SpreadsheetDocument document)
    {
        var packageProperties = document.PackageProperties;

        if (!string.IsNullOrEmpty(_fileProperties.Title))
        {
            packageProperties.Title = _fileProperties.Title;
        }

        if (!string.IsNullOrEmpty(_fileProperties.Subject))
        {
            packageProperties.Subject = _fileProperties.Subject;
        }

        if (!string.IsNullOrEmpty(_fileProperties.Creator))
        {
            packageProperties.Creator = _fileProperties.Creator;
        }

        if (!string.IsNullOrEmpty(_fileProperties.Keywords))
        {
            packageProperties.Keywords = _fileProperties.Keywords;
        }

        if (!string.IsNullOrEmpty(_fileProperties.Description))
        {
            packageProperties.Description = _fileProperties.Description;
        }

        if (!string.IsNullOrEmpty(_fileProperties.Version))
        {
            packageProperties.LastModifiedBy = _fileProperties.Version;
        }

        if (!string.IsNullOrEmpty(_fileProperties.Revision))
        {
            packageProperties.Revision = _fileProperties.Revision;
        }

        if (!string.IsNullOrEmpty(_fileProperties.Category))
        {
            packageProperties.Category = _fileProperties.Category;
        }
    }
    ...
    private DropdownSheetInfo? CreateDropdownSheetForGroup(WorkbookPart workbookPart, IGrouping<string, ISheetConfiguration> configGroup, CancellationToken cancellationToken)
    {
        ...
        // Use the same _cellWriter instance that was initialized in BuildExcelDocument
...
/// <summary>
/// Factory interface for creating Excel file builders
/// </summary>
public interface IExcelFileBuilderFactory
{
    /// <summary>
    /// Creates a new instance of Excel file builder
    /// </summary>
    /// <returns>A new Excel file builder instance</returns>
    IExcelFileBuilder Create();

    /// <summary>
    /// Creates a new instance of Excel file builder with custom dependencies
    /// </summary>
    /// <param name="cacheManager">Cache manager for performance optimization</param>
    /// <param name="cellWriter">Cell writer for data output</param>
    /// <param name="styleManager">Style manager for formatting</param>
    /// <param name="validationManager">Validation manager for data validation</param>
    /// <param name="stringBuilderPool">String builder pool for performance</param>
    /// <param name="propertyNameFormatter">Formatter for property names</param>
    /// <param name="workbookWriter">Workbook writer for final output</param>
    /// <param name="sensitivityManager">Sensitivity manager for security labels</param>
    /// <returns>A new Excel file builder instance with custom dependencies</returns>
    IExcelFileBuilder Create(
        IExcelCacheManager cacheManager,
        IExcelCellWriter cellWriter,
        IExcelStyleManager styleManager,
...
public sealed class ExcelFileBuilderFactory : IExcelFileBuilderFactory
{
    private readonly IServiceProvider? _serviceProvider;

    public ExcelFileBuilderFactory(IServiceProvider? serviceProvider = null)
    {
        _serviceProvider = serviceProvider;
    }

    public IExcelFileBuilder Create()
    {
        if (_serviceProvider != null)
        {
            var builder = _serviceProvider.GetService(typeof(IExcelFileBuilder)) as IExcelFileBuilder;
            if (builder != null)
            {
                return builder;
            }
...
public sealed class HeaderRowWriter
{
    ...
    public HeaderRowWriter(IExcelCacheManager cacheManager, IExcelCellWriter cellWriter, StringBuilderPool stringBuilderPool)
    {
        _cacheManager = cacheManager ?? throw new ArgumentNullException(nameof(cacheManager));
        _cellWriter = cellWriter ?? throw new ArgumentNullException(nameof(cellWriter));
        _stringBuilderPool = stringBuilderPool ?? throw new ArgumentNullException(nameof(stringBuilderPool));
    ...
    public void WriteHeader(OpenXmlWriter writer, PropertyAccessor[] propertyAccessors, Dictionary<string, uint> styleMappings, uint rowIndex)
    {
        ...
        try
        {
            for (var i = 0; i < propertyAccessors.Length; i++)
            {
                ...
                // Use cellWriter which now handles SharedStringTable
                _cellWriter.WriteCellValue(writer, cellReference, accessor.ColumnName, typeof(string), styleIndex);
            }
        }
        finally
        {
...
/// <summary>
/// Interface for configuring individual sheets in an Excel file
/// </summary>
/// <typeparam name="T">The type of data model for the sheet</typeparam>
public interface ISheetBuilder<T> where T : class, new()
{
    /// <summary>
    /// Sets the data for the sheet
    /// </summary>
    /// <param name="data">List of data items</param>
    /// <returns>The sheet builder for method chaining</returns>
    ISheetBuilder<T> WithData(List<T> data);

    /// <summary>
    /// Excludes specified properties from the sheet
    /// </summary>
    /// <param name="excludeProperties">Properties to exclude</param>
    /// <returns>The sheet builder for method chaining</returns>
    ISheetBuilder<T> ExcludeProperties(params Expression<Func<T, object>>[] excludeProperties);

    /// <summary>
    /// Adds dropdown data configuration to the sheet
    /// </summary>
    /// <param name="dropdownData">Dropdown data configuration</param>
    /// <param name="dropdownSheetName">Name of the dropdown sheet</param>
    /// <returns>The sheet builder for method chaining</returns>
    ISheetBuilder<T> WithDropdownData(List<ExcelDropdownData> dropdownData, string dropdownSheetName = "Static Data");

    /// <summary>
    /// Adds conditional formatting rules to the sheet
    /// </summary>
    /// <param name="conditionalFactory">Factory for creating conditional rules</param>
    /// <returns>The sheet builder for method chaining</returns>
    ISheetBuilder<T> WithConditionalRules(IExcelConditionalRulesFactory<T> conditionalFactory);

    /// <summary>
    /// Completes sheet configuration and returns to the file builder
    /// </summary>
    /// <returns>The parent file builder</returns>
    IExcelFileBuilder Done();
}
...
public sealed class SheetConfiguration<T> : ISheetConfiguration where T : class, new()
{
        ...
        IExcelCacheManager cacheManager, IExcelCellWriter cellWriter, StringBuilderPool stringBuilderPool, CancellationToken cancellationToken)
    {
        ...
        // Updated to pass cellWriter to HeaderRowWriter
        var headerWriter = new HeaderRowWriter(cacheManager, cellWriter, stringBuilderPool);
    ...
    private void WriteStaticHeaderRow(OpenXmlWriter writer, List<ExcelDropdownData> dropdownDataList, uint rowIndex)
    {
        ...
        try
        {
            for (var colIndex = 0; colIndex < dropdownDataList.Count; colIndex++)
            {
                ...
                // Use cellWriter to handle SharedStringTable for headers
                _cellWriter.WriteCellValue(writer, cellReference, column.ColumnName, typeof(string), 1);
            }
        }
        finally
        {
        ...
        StringBuilder stringBuilder)
    {
        ...
        for (var colIndex = 0; colIndex < dropdownDataList.Count; colIndex++)
        {
            ...
            if (rowOffset < columnData.Length)
            {
                ...
                // Use cellWriter which handles SharedStringTable
