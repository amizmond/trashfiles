...
public class FeatureSnapshotService : IFeatureSnapshotService
{
    ...
    private readonly IReadOnlyList<IFeatureSnapshotDeletionGuard> _deletionGuards;

    public FeatureSnapshotService(
        IDbContextFactory<EstimationDbContext> ctx,
        IAuditUserProvider auditUser,
        IEnumerable<IFeatureSnapshotDeletionGuard>? deletionGuards = null)
    {
        _ctx = ctx;
        _auditUser = auditUser;
        _deletionGuards = deletionGuards?.ToList() ?? [];
    }
    ...
    public async Task<bool> DeleteAsync(int id)
    {
        ...
        foreach (var guard in _deletionGuards)
        {
            if (await guard.GetBlockingReasonAsync(id) is { } reason)
            {
