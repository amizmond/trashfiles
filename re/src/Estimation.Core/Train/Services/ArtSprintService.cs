...
public class ArtSprintPlan
{
    ...
    public int PlannedFeatureCount { get; set; }
...
{
    Task<List<ArtSprint>> GetForProjectAsync(int capitalProjectId);
    Task<ArtSprintPlan> PlanSaveAsync(ArtSprint entity);
    Task<ArtSprintPlan> SaveAsync(ArtSprint entity);
...
public class ArtSprintService : IArtSprintService
{
    ...
    public async Task<List<ArtSprint>> GetForProjectAsync(int capitalProjectId)
    {
    ...
    public async Task<ArtSprintPlan> PlanSaveAsync(ArtSprint entity)
    {
        await using var db = await _ctx.CreateDbContextAsync();
        Normalize(entity);
    ...
    public async Task<ArtSprintPlan> SaveAsync(ArtSprint entity)
    {
        await using var db = await _ctx.CreateDbContextAsync();
        Normalize(entity);
        var plan = await ValidateAsync(db, entity);
        ...
        if (entity.Id == 0)
        {
            stored = new ArtSprint
            {
                ...
                UatStart = entity.UatStart,
                UatEnd = entity.UatEnd,
                FixVersion = entity.FixVersion,
        ...
        else
        {
            ...
            stored.UatStart = entity.UatStart;
            stored.UatEnd = entity.UatEnd;
            stored.FixVersion = entity.FixVersion;
            stored.SignOffDate = entity.SignOffDate;
            stored.ReleaseDate = entity.ReleaseDate;
        ...
        foreach (var team in teams)
        {
            ...
            var featurePlans = await db.FeatureTeamSprints.Where(x => x.TeamId == team.Id).ToListAsync();
            ApplyTeamActions(plan, db, team, stored, sprints, featurePlans);
        }
    ...
    public async Task<ArtSprintPlan> PlanDeleteAsync(int id)
    {
        ...
        plan.PlannedFeatureCount = await db.FeatureTeamSprints
            .Where(x => x.Sprint.SourceArtSprintId == id)
    ...
    public async Task<ArtSprintPlan> BackfillTeamAsync(int capitalProjectId, int teamId)
    {
        ...
        var featurePlans = await db.FeatureTeamSprints.Where(x => x.TeamId == teamId).ToListAsync();
        foreach (var artSprint in artSprints)
        {
            ApplyTeamActions(plan, db, team, artSprint, sprints, featurePlans);
        }
    ...
    private static Task<List<ArtSprint>> LoadArtSprintsAsync(EstimationDbContext db, int capitalProjectId)
    {
    ...
    private static void Normalize(ArtSprint entity)
    {
    ...
    private static async Task<ArtSprintPlan> ValidateAsync(EstimationDbContext db, ArtSprint entity)
    {
        var plan = new ArtSprintPlan();
        ...
        {
            plan.Errors.Add("Dev end must be after dev start.");
        }

        plan.Errors.AddRange(SprintUatRules.Validate(entity.EndDate, entity.UatStart, entity.UatEnd, entity.FixVersion));
    ...
    private static string DerivedName(Team team, ArtSprint artSprint)
    {
    ...
    private static bool HasRequiredFormat(Sprint sprint, Team team, ArtSprint artSprint)
    {
    ...
    private static (Sprint? Winner, List<Sprint> Losers) Partition(
        List<Sprint> teamSprints, ArtSprint artSprint)
    {
    ...
    private static void BuildTeamActions(
        ArtSprintPlan plan, Team team, ArtSprint artSprint, List<Sprint> teamSprints)
    {
    ...
    private static void ApplyTeamActions(
        ...
        Team team,
        ArtSprint artSprint,
        List<Sprint> teamSprints,
        List<FeatureTeamSprint> featurePlans)
    {
        var (existingWinner, losers) = Partition(teamSprints, artSprint);

        var created = existingWinner is null;
        var winner = existingWinner ?? new Sprint { TeamId = team.Id, Name = DerivedName(team, artSprint) };
        if (created)
        {
            db.Sprints.Add(winner);
            teamSprints.Add(winner);
        }
        else if (!HasRequiredFormat(winner, team, artSprint))
        {
        ...
        foreach (var loser in losers)
        {
            ...
            winner.ColorHex ??= loser.ColorHex;
            MoveFeaturePlans(db, featurePlans, loser, winner);
            plan.Actions.Add(new ArtSprintCascadeAction(
                ArtSprintCascadeKind.Deleted, team.Name, loser.Name, loser.StartDate, loser.EndDate));
        ...
        plan.Actions.Add(new ArtSprintCascadeAction(
            created ? ArtSprintCascadeKind.Created : ArtSprintCascadeKind.Updated,
            team.Name, winner.Name, artSprint.StartDate, artSprint.EndDate));
        ...
        winner.UatStart = artSprint.UatStart;
        winner.UatEnd = artSprint.UatEnd;
        winner.FixVersion = artSprint.FixVersion;
        winner.SignOffDate = artSprint.SignOffDate;
        winner.ReleaseDate = artSprint.ReleaseDate;
    }

    private static void MoveFeaturePlans(
        EstimationDbContext db, List<FeatureTeamSprint> featurePlans, Sprint from, Sprint to)
    {
        foreach (var existing in featurePlans.Where(p => p.SprintId == from.Id).ToList())
        {
            var alreadyPlanned = featurePlans.Any(p =>
                p.FeatureId == existing.FeatureId
                && (ReferenceEquals(p.Sprint, to) || (to.Id != 0 && p.SprintId == to.Id)));
            if (!alreadyPlanned)
            {
                var moved = new FeatureTeamSprint { FeatureId = existing.FeatureId, TeamId = existing.TeamId, Sprint = to };
                db.FeatureTeamSprints.Add(moved);
                featurePlans.Add(moved);
            }

            db.FeatureTeamSprints.Remove(existing);
            featurePlans.Remove(existing);
        }
    }
}
