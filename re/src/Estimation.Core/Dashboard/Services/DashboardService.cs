...
namespace Estimation.Core.Dashboard.Services;

public record ChartItem(string Label, double Value);

public record PiTimelineItem(string Name, DateTime? StartDate, DateTime? EndDate, bool IsCurrent);

public record ArtSprintTimelineItem(
    int Id,
    string Name,
    string? PiName,
    DateTime DevStart,
...
public record ArtCoverageItem(string Name, int ProgramCount, int TeamCount, int FeatureCount);

public record DashboardArtOption(int Id, string Name);
...
public record DashboardData(
    ...
    List<ArtCoverageItem> CapitalProjectCoverage,
...
public interface IDashboardService
{
    Task<DashboardData> GetDashboardDataAsync(
        ...
        IReadOnlyCollection<int>? allowedCapitalProjectIds,
        CancellationToken ct = default);

    Task<List<DashboardArtOption>> GetCapitalProjectOptionsAsync(
        IReadOnlyCollection<int>? allowedCapitalProjectIds,
        CancellationToken ct = default);

    Task<List<PiTimelineItem>> GetPiTimelineAsync(CancellationToken ct = default);

    Task<List<DashboardArtOption>> GetArtSprintOptionsAsync(
        IReadOnlyCollection<int>? allowedCapitalProjectIds,
        CancellationToken ct = default);

    Task<List<ArtSprintTimelineItem>> GetArtSprintTimelineAsync(int capitalProjectId, CancellationToken ct = default);
}
...
public class DashboardService : IDashboardService
{
    ...
    public Task<List<DashboardArtOption>> GetCapitalProjectOptionsAsync(
        IReadOnlyCollection<int>? allowedCapitalProjectIds,
        CancellationToken ct = default)
    {
        return GetCachedAsync(
            $"dash:capitalProjectOptions{ScopeSuffix(allowedCapitalProjectIds)}",
            (db, t) => LoadCapitalProjectOptionsAsync(db, allowedCapitalProjectIds, t),
            ct);
    }

    private static async Task<List<DashboardArtOption>> LoadCapitalProjectOptionsAsync(
        EstimationDbContext db, IReadOnlyCollection<int>? allowedCapitalProjectIds, CancellationToken ct)
    {
        var query = db.CapitalProjects.AsNoTracking();
        if (allowedCapitalProjectIds is not null)
        {
            var allowed = allowedCapitalProjectIds.ToList();
            query = query.Where(cp => allowed.Contains(cp.Id));
        }

        return await query
            .OrderBy(cp => cp.Name)
            .Select(cp => new DashboardArtOption(cp.Id, cp.Name))
            .ToListAsync(ct);
    }
    ...
    public async Task<DashboardData> GetDashboardDataAsync(
        ...
        CancellationToken ct = default)
    {
        var scope = ResolveScope(capitalProjectId, allowedCapitalProjectIds);
        var filterSuffix = ScopeSuffix(scope);

        List<int>? featureIds = null;
        if (scope is not null)
        {
            featureIds = await GetCachedAsync(
                $"dash:featureIds{filterSuffix}",
                (db, t) => GetFilteredFeatureIdsAsync(db, scope, t),
                ct);
        }
        ...
        var piTimelineTask = GetPiTimelineAsync(ct);
    ...
    private static IReadOnlyCollection<int>? ResolveScope(
        int? capitalProjectId, IReadOnlyCollection<int>? allowedCapitalProjectIds)
    {
        if (capitalProjectId is not { } selected)
        {
            return allowedCapitalProjectIds;
        }

        if (allowedCapitalProjectIds is not null && !allowedCapitalProjectIds.Contains(selected))
        {
            throw new UnauthorizedAccessException($"No access to capital project {selected}.");
        }

        return new[] { selected };
    }

    private static string ScopeSuffix(IReadOnlyCollection<int>? scope) =>
        scope is null ? ":all" : $":{string.Join('-', scope.OrderBy(id => id))}";

    private static async Task<List<int>> GetFilteredFeatureIdsAsync(
        EstimationDbContext db, IReadOnlyCollection<int> capitalProjectIds, CancellationToken ct)
    {
        var ids = capitalProjectIds.ToList();
            ...
            .Where(f =>
                ...
                f.BusinessOutcome.PortfolioEpic.StrategicObjectivePortfolioEpics
                    .Any(ppe => ppe.StrategicObjective.CapitalProjectStrategicObjectives
                        .Any(cpp => ids.Contains(cpp.CapitalProjectId))))
            .Select(f => f.Id)
            .ToListAsync(ct);
    }

    private static IQueryable<Feature> FilterByIds(EstimationDbContext db, List<int>? featureIds)
    {
    ...
    {
        IQueryable<FeatureTeam> query = db.FeatureTeams.AsNoTracking();
        if (featureIds is not null)
        {
    ...
    public Task<List<PiTimelineItem>> GetPiTimelineAsync(CancellationToken ct = default) =>
        GetCachedAsync("dash:piTimeline", LoadPiTimelineAsync, ct);

    public Task<List<DashboardArtOption>> GetArtSprintOptionsAsync(
        IReadOnlyCollection<int>? allowedCapitalProjectIds,
        CancellationToken ct = default)
    {
        return GetCachedAsync(
            $"dash:artSprintOptions{ScopeSuffix(allowedCapitalProjectIds)}",
            (db, t) => LoadArtSprintOptionsAsync(db, allowedCapitalProjectIds, t),
            ct);
    }

    public Task<List<ArtSprintTimelineItem>> GetArtSprintTimelineAsync(int capitalProjectId, CancellationToken ct = default) =>
        GetCachedAsync($"dash:artSprints:{capitalProjectId}", (db, t) => LoadArtSprintTimelineAsync(db, capitalProjectId, t), ct);

    private static async Task<List<DashboardArtOption>> LoadArtSprintOptionsAsync(
        EstimationDbContext db, IReadOnlyCollection<int>? allowedCapitalProjectIds, CancellationToken ct)
    {
        var query = db.CapitalProjects
            .AsNoTracking()
            .Where(cp => db.CapitalProjectSprints.Any(s => s.CapitalProjectId == cp.Id));
        if (allowedCapitalProjectIds is not null)
        {
            var allowed = allowedCapitalProjectIds.ToList();
            query = query.Where(cp => allowed.Contains(cp.Id));
    ...
    private static async Task<List<ArtSprintTimelineItem>> LoadArtSprintTimelineAsync(
        EstimationDbContext db, int capitalProjectId, CancellationToken ct)
    {
        var today = DateTime.Today;

        var raw = await db.CapitalProjectSprints
            .AsNoTracking()
        ...
        return raw.Select(r => new ArtSprintTimelineItem(
            r.Id, r.Name, r.PiName, r.StartDate, r.EndDate, r.UatStart, r.UatEnd,
            r.SignOffDate, r.ReleaseDate, r.FixVersion, r.IsIpSprint,
            r.StartDate.Date <= today && today <= r.EndDate.Date,
            r.EndDate.Date < today)).ToList();
    }

    private static async Task<List<PiTimelineItem>> LoadPiTimelineAsync(EstimationDbContext db, CancellationToken ct)
    {
        var today = DateTime.Today;
    ...
    private static async Task<List<ChartItem>> GetResourcesByLocationAsync(EstimationDbContext db, CancellationToken ct)
    {
        var raw = await db.HumanResources
            ...
            .GroupBy(hr => hr.City!.Country.Name)
            .Select(g => new { Label = g.Key, Count = g.Count() })
            .OrderByDescending(c => c.Count)
            .ToListAsync(ct);
        return raw.Select(r => new ChartItem(r.Label, r.Count)).ToList();
    ...
    private static async Task<List<ArtCoverageItem>> GetCapitalProjectCoverageAsync(EstimationDbContext db, CancellationToken ct)
    {
        var raw = await db.CapitalProjects
            ...
            .OrderBy(cp => cp.Name)
            .ToListAsync(ct);
        return raw.Select(r => new ArtCoverageItem(r.Name, r.ProgramCount, r.TeamCount, r.FeatureCount)).ToList();
    }
