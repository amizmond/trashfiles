...
public class ArtSprintServiceTests
{
    ...
    {
        db.CapitalProjects.Add(new Art { Id = 1, Name = "Atlas" });
        db.Pis.Add(new Pi { Id = 7, Name = "2026.PI1", StartDate = PiStart, EndDate = PiEnd });

        foreach (var team in teams)
        {
            db.Teams.Add(team);
            db.CapitalProjectTeams.Add(new ArtTeam { CapitalProjectId = 1, TeamId = team.Id });
        }
    });

    private static ArtSprint Sprint(string name, DateTime start, DateTime end, int id = 0) => new()
    {
        Id = id,
        CapitalProjectId = 1,
        PiId = 7,
    ...
    public async Task An_end_date_on_or_before_the_start_date_is_rejected()
    {
        ...
        Assert.True(plan.HasErrors);
        Assert.Contains(plan.Errors, e => e.Contains("Dev end must be after dev start"));
    }
    ...
    public async Task Backfill_applies_every_existing_art_sprint_to_a_newly_added_team()
    {
        ...
        await _db.SeedAsync(db =>
        {
            ...
            db.CapitalProjectTeams.Add(new ArtTeam { CapitalProjectId = 1, TeamId = 11 });
        });
    ...
    [Fact]
    public async Task Uat_and_fix_version_cascade_to_team_sprints()
    {
        await SeedAsync(new Team { Id = 10, Name = "Axis" });
        var art = Sprint("186", new DateTime(2026, 1, 5), new DateTime(2026, 1, 18));
        art.UatStart = new DateTime(2026, 1, 19);
        art.UatEnd = new DateTime(2026, 1, 30);
        art.FixVersion = " R26.1 ";

        await _service.SaveAsync(art);

        var sprint = Assert.Single(await TeamSprintsAsync(10));
        Assert.Equal(new DateTime(2026, 1, 19), sprint.UatStart);
        Assert.Equal(new DateTime(2026, 1, 30), sprint.UatEnd);
        Assert.Equal("R26.1", sprint.FixVersion);
    }

    [Fact]
    public async Task Editing_uat_on_an_art_sprint_updates_the_team_sprints()
    {
        await SeedAsync(new Team { Id = 10, Name = "Axis" });
        var art = Sprint("186", new DateTime(2026, 1, 5), new DateTime(2026, 1, 18));
        await _service.SaveAsync(art);

        art.UatEnd = new DateTime(2026, 2, 6);
        art.FixVersion = "R26.2";
        await _service.SaveAsync(art);

        var sprint = Assert.Single(await TeamSprintsAsync(10));
        Assert.Equal(new DateTime(2026, 2, 6), sprint.UatEnd);
        Assert.Equal("R26.2", sprint.FixVersion);
    }

    [Fact]
    public async Task A_uat_that_ends_before_the_sprint_is_rejected()
    {
        await SeedAsync(new Team { Id = 10, Name = "Axis" });
        var art = Sprint("186", new DateTime(2026, 1, 5), new DateTime(2026, 1, 18));
        art.UatStart = new DateTime(2026, 1, 10);
        art.UatEnd = new DateTime(2026, 1, 15);

        var plan = await _service.SaveAsync(art);

        Assert.True(plan.HasErrors);
        Assert.Contains(plan.Errors, e => e.Contains("UAT cannot end before dev end"));
        Assert.Empty(await TeamSprintsAsync(10));
    }

    [Fact]
    public async Task A_uat_end_before_its_start_is_rejected()
    {
        await SeedAsync(new Team { Id = 10, Name = "Axis" });
        var art = Sprint("186", new DateTime(2026, 1, 5), new DateTime(2026, 1, 18));
        art.UatStart = new DateTime(2026, 1, 28);
        art.UatEnd = new DateTime(2026, 1, 20);

        var plan = await _service.SaveAsync(art);

        Assert.Contains(plan.Errors, e => e.Contains("UAT end must be on or after UAT start"));
    }

    [Fact]
    public async Task Missing_uat_dates_are_rejected()
    {
        await SeedAsync(new Team { Id = 10, Name = "Axis" });
        var art = Sprint("186", new DateTime(2026, 1, 5), new DateTime(2026, 1, 18));
        art.UatStart = default;
        art.UatEnd = default;

        var plan = await _service.SaveAsync(art);

        Assert.Contains(plan.Errors, e => e.Contains("UAT start and UAT end are required"));
    }

    [Fact]
    public async Task Uat_windows_may_overlap_the_next_sprint()
    {
        await SeedAsync(new Team { Id = 10, Name = "Axis" });
        var first = Sprint("186", new DateTime(2026, 1, 5), new DateTime(2026, 1, 18));
        first.UatEnd = new DateTime(2026, 1, 28);
        await _service.SaveAsync(first);

        var plan = await _service.SaveAsync(Sprint("187", new DateTime(2026, 1, 19), new DateTime(2026, 2, 1)));

        Assert.False(plan.HasErrors);
        Assert.Equal(2, (await TeamSprintsAsync(10)).Count);
    }

    [Fact]
    public async Task Feature_plans_on_a_dropped_sprint_move_to_its_replacement()
    {
        await SeedAsync(new Team { Id = 10, Name = "Axis" });
        await _db.SeedAsync(db =>
        {
            db.Sprints.Add(new Sprint
            {
                ...
                StartDate = new DateTime(2026, 1, 1),
                EndDate = new DateTime(2026, 1, 10),
                UatStart = new DateTime(2026, 1, 11),
                UatEnd = new DateTime(2026, 1, 12),
            });
            db.Features.Add(new Feature { Id = 1, Summary = "Planned feature" });
            db.FeatureTeams.Add(new FeatureTeam { FeatureId = 1, TeamId = 10 });
            db.FeatureTeamSprints.Add(new FeatureTeamSprint { FeatureId = 1, TeamId = 10, SprintId = 90 });
        });

        await _service.SaveAsync(Sprint("186", new DateTime(2026, 1, 5), new DateTime(2026, 1, 18)));

        var replacement = Assert.Single(await TeamSprintsAsync(10));
        var plan = Assert.Single(await _db.ReadAsync(db => db.FeatureTeamSprints.ToListAsync()));
        Assert.NotEqual(90, replacement.Id);
        Assert.Equal(replacement.Id, plan.SprintId);
        Assert.Equal(1, plan.FeatureId);
    }

    [Fact]
    public async Task Deleting_an_art_sprint_reports_how_many_features_planned_it()
    {
        await SeedAsync(new Team { Id = 10, Name = "Axis" });
        var art = Sprint("186", new DateTime(2026, 1, 5), new DateTime(2026, 1, 18));
        await _service.SaveAsync(art);
        var teamSprint = Assert.Single(await TeamSprintsAsync(10));
        await _db.SeedAsync(db =>
        {
            db.Features.Add(new Feature { Id = 1, Summary = "A" });
            db.Features.Add(new Feature { Id = 2, Summary = "B" });
            db.FeatureTeams.Add(new FeatureTeam { FeatureId = 1, TeamId = 10 });
            db.FeatureTeams.Add(new FeatureTeam { FeatureId = 2, TeamId = 10 });
            db.FeatureTeamSprints.Add(new FeatureTeamSprint { FeatureId = 1, TeamId = 10, SprintId = teamSprint.Id });
            db.FeatureTeamSprints.Add(new FeatureTeamSprint { FeatureId = 2, TeamId = 10, SprintId = teamSprint.Id });
        });

        var plan = await _service.PlanDeleteAsync(art.Id);

        Assert.Equal(2, plan.PlannedFeatureCount);
    }

    [Fact]
    public async Task Sign_off_and_release_dates_cascade_to_team_sprints()
    {
        await SeedAsync(new Team { Id = 10, Name = "Axis" });
        var art = Sprint("186", new DateTime(2026, 1, 5), new DateTime(2026, 1, 18));
        art.SignOffDate = new DateTime(2026, 1, 28, 15, 30, 0);
        art.ReleaseDate = new DateTime(2026, 2, 2);

        await _service.SaveAsync(art);

        var sprint = Assert.Single(await TeamSprintsAsync(10));
        Assert.Equal(new DateTime(2026, 1, 28), sprint.SignOffDate);
        Assert.Equal(new DateTime(2026, 2, 2), sprint.ReleaseDate);
    }

    [Fact]
    public async Task Clearing_sign_off_and_release_on_the_art_sprint_clears_them_on_team_sprints()
    {
        await SeedAsync(new Team { Id = 10, Name = "Axis" });
        var art = Sprint("186", new DateTime(2026, 1, 5), new DateTime(2026, 1, 18));
        art.SignOffDate = new DateTime(2026, 1, 28);
        art.ReleaseDate = new DateTime(2026, 2, 2);
        await _service.SaveAsync(art);

        art.SignOffDate = null;
        art.ReleaseDate = null;
        await _service.SaveAsync(art);

        var sprint = Assert.Single(await TeamSprintsAsync(10));
        Assert.Null(sprint.SignOffDate);
        Assert.Null(sprint.ReleaseDate);
    }
}
