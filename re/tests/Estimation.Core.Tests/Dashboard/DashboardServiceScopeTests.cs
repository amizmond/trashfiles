...
public class DashboardServiceScopeTests
{
    ...
    private Task SeedAsync() => _db.SeedAsync(db =>
    {
        ...
        db.CapitalProjects.Add(new Art { Id = Alpha, Name = "ART Alpha" });
        db.CapitalProjects.Add(new Art { Id = Beta, Name = "ART Beta" });
    ...
    [Fact]
    public async Task The_PI_timeline_is_the_same_whatever_the_caller_may_see()
    {
        await SeedAsync();
        var service = NewService();

        var restricted = await service.GetPiTimelineAsync();
        var pi = Assert.Single(restricted);

        Assert.Equal("PI-1", pi.Name);
        Assert.Equal(new DateTime(2026, 1, 1), pi.StartDate);
        Assert.Equal(new DateTime(2026, 3, 31), pi.EndDate);

        var viaDashboard = await service.GetDashboardDataAsync(null, new[] { Alpha });
        Assert.Equal(restricted, viaDashboard.PiTimeline);
    }
