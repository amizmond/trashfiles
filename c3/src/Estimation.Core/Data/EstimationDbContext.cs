...
public class EstimationDbContext : DbContext
{
    ...
    public DbSet<Art> CapitalProjects => Set<Art>();
    ...
    public DbSet<ArtStrategicObjective> CapitalProjectStrategicObjectives => Set<ArtStrategicObjective>();
    public DbSet<ArtTeam> CapitalProjectTeams => Set<ArtTeam>();
    public DbSet<ArtJiraKey> CapitalProjectJiraKeys => Set<ArtJiraKey>();
    public DbSet<ArtSprint> CapitalProjectSprints => Set<ArtSprint>();
    ...
    public DbSet<FeatureTeamSprint> FeatureTeamSprints => Set<FeatureTeamSprint>();
    ...
    public DbSet<GlobalMessage> GlobalMessages => Set<GlobalMessage>();

    public DbSet<JiraSyncSettings> JiraSyncSettings => Set<JiraSyncSettings>();
    public DbSet<JiraSyncKey> JiraSyncKeys => Set<JiraSyncKey>();
    ...
    public DbSet<RiskArt> RiskCapitalProjects => Set<RiskArt>();
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Team>(e =>
        {
            ...
            e.Property(t => t.JiraName).HasMaxLength(50);
        ...
        modelBuilder.Entity<ArtJiraKey>(e =>
        {
            e.ToTable("CapitalProjectJiraKeys");
            e.HasKey(k => k.Id);
            e.Property(k => k.JiraKey).IsRequired().HasMaxLength(ArtJiraKey.MaxKeyLength);
            e.Property(k => k.Components).HasMaxLength(ArtJiraKey.MaxFilterLength);
            e.Property(k => k.Labels).HasMaxLength(ArtJiraKey.MaxFilterLength);
            e.HasOne(k => k.Art).WithMany(cp => cp.JiraKeys)
             .HasForeignKey(k => k.CapitalProjectId).OnDelete(DeleteBehavior.Cascade);
            e.HasIndex(k => new { k.CapitalProjectId, k.JiraKey }).IsUnique();
            e.HasIndex(k => k.JiraKey);
        });

        modelBuilder.Entity<ArtStrategicObjective>(e =>
        {
            e.ToTable("CapitalProjectStrategicObjectives");
            e.HasKey(cpp => new { cpp.CapitalProjectId, cpp.StrategicObjectiveId });
            e.HasOne(cpp => cpp.Art).WithMany(cp => cp.CapitalProjectStrategicObjectives)
        ...
        modelBuilder.Entity<ArtTeam>(e =>
        {
            e.HasKey(cpt => new { cpt.CapitalProjectId, cpt.TeamId });
            e.HasOne(cpt => cpt.Art).WithMany(cp => cp.CapitalProjectTeams)
        ...
        modelBuilder.Entity<FeatureTeamSprint>(e =>
        {
            e.HasKey(x => new { x.FeatureId, x.TeamId, x.SprintId });
            e.HasOne(x => x.FeatureTeam).WithMany(ft => ft.Sprints)
        ...
        modelBuilder.Entity<JiraLabelCache>(e =>
        {
            ...
            e.Property(c => c.LastError).HasMaxLength(JiraLabelCache.MaxErrorLength);
        ...
        modelBuilder.Entity<AppUser>(e =>
        {
            ...
            e.Property(u => u.RequestComment).HasMaxLength(AppUser.MaxRequestCommentLength);

            e.HasOne<Department>().WithMany()
             .HasForeignKey(u => u.RequestedDepartmentId).OnDelete(DeleteBehavior.SetNull);
            e.HasOne<Art>().WithMany()
             .HasForeignKey(u => u.RequestedCapitalProjectId).OnDelete(DeleteBehavior.SetNull);
        ...
        modelBuilder.Entity<AppPage>(e =>
        {
            ...
            e.HasData(
                ...
                new AppPage { Id = 29, Key = "FeatureHygiene", DisplayName = "Feature Hygiene", Group = "Planning", IsAdminOnly = false, SortOrder = 27, ScopeMode = PageScopeMode.TrainScoped },
                new AppPage { Id = 30, Key = "General", DisplayName = "General", Group = "Admin", IsAdminOnly = true, SortOrder = 89, ScopeMode = PageScopeMode.None }
            );
        });

        modelBuilder.Entity<GlobalMessage>(e =>
        {
            e.HasKey(m => m.Id);
            e.Property(m => m.Text).HasMaxLength(GlobalMessage.MaxTextLength);
            e.Property(m => m.UpdatedBy).HasMaxLength(256);
            e.Ignore(m => m.IsVisible);
        });
        ...
        modelBuilder.Entity<JiraSyncKey>(e =>
        {
            e.HasKey(x => x.Id);
            e.Property(x => x.JiraKey).IsRequired().HasMaxLength(ArtJiraKey.MaxKeyLength);
            ...
            e.HasIndex(x => x.JiraKey).IsUnique();
        });
        ...
        modelBuilder.Entity<Sprint>(e =>
        {
            ...
            e.Property(x => x.FixVersion).HasMaxLength(50);
            ...
            e.HasOne(x => x.SourceArtSprint).WithMany()
             .HasForeignKey(x => x.SourceArtSprintId).OnDelete(DeleteBehavior.SetNull).IsRequired(false);

            e.HasIndex(x => x.TeamId);
            e.HasIndex(x => new { x.TeamId, x.StartDate, x.EndDate });
            e.HasIndex(x => x.PiId);
            e.HasIndex(x => x.SourceArtSprintId);
        });

        modelBuilder.Entity<ArtSprint>(e =>
        {
            e.HasKey(x => x.Id);
            e.Property(x => x.Name).IsRequired().HasMaxLength(20);
            e.Property(x => x.FixVersion).HasMaxLength(50);

            e.HasOne(x => x.Art).WithMany()
             .HasForeignKey(x => x.CapitalProjectId).OnDelete(DeleteBehavior.Cascade);
            e.HasOne(x => x.Pi).WithMany()
             .HasForeignKey(x => x.PiId).OnDelete(DeleteBehavior.Restrict);

            e.HasIndex(x => new { x.CapitalProjectId, x.Name }).IsUnique();
            e.HasIndex(x => new { x.CapitalProjectId, x.StartDate, x.EndDate });
            e.HasIndex(x => x.PiId);
        });
        ...
        modelBuilder.Entity<FeatureSnapshot>(e =>
        {
            ...
            e.Property(s => s.ArtJiraKeys).HasMaxLength(FeatureSnapshot.ArtJiraKeysMaxLength);
