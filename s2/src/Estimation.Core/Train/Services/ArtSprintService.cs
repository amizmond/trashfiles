...
public interface IArtSprintService
{
    ...
    Task<List<CapitalProjectSprint>> GetForProjectAndPiAsync(int capitalProjectId, int piId);
...
public class ArtSprintService : IArtSprintService
{
    ...
    public async Task<List<CapitalProjectSprint>> GetForProjectAndPiAsync(int capitalProjectId, int piId)
    {
        await using var db = await _ctx.CreateDbContextAsync();
        return await db.CapitalProjectSprints
            .Where(s => s.CapitalProjectId == capitalProjectId && s.PiId == piId)
            .AsNoTracking()
