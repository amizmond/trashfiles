...
public class JiraIssueService : IJiraIssueService
{
    ...
    public async Task<string> CreateIssueAsync(string userName, JiraCreateIssueRequest request)
    {
        ...
        if (!string.IsNullOrEmpty(request.ParentJiraKey) && !string.IsNullOrEmpty(_settings.ParentLinkCustomFieldId))
        {
            fields[_settings.ParentLinkCustomFieldId] = request.ParentJiraKey;
        }
