...
public partial class ArtService : IArtService
{
        ...
        rows.Select(r => new ArtJiraKey
            {
                ...
                Components = JiraValueFilter.Parse(r.Components).ToCsv(),
                Labels = JiraValueFilter.Parse(r.Labels).ToCsv()
            })
        ...
        EstimationDbContext db, int artId, IReadOnlyCollection<ArtJiraKey> changedRows, IReadOnlyCollection<string> addedKeys)
    {
        ...
        var problems = changedRows
            .SelectMany(r => new[]
            {
                JiraValueFilter.Parse(r.Components).Problem("components"),
                JiraValueFilter.Parse(r.Labels).Problem("labels")
            }.OfType<string>().Select(p => $"Jira key {r.JiraKey}: {p}"))
            .ToList();
        if (problems.Count > 0)
        {
