...
public static class ArtHelper
{
    ...
    private static string Requirement(ArtKeyScope scope)
    {
        ...
        if (scope.ComponentFilter.IsActive)
        {
            parts.Add(scope.ComponentFilter.Requirement("component", "components"));
        }
        if (scope.LabelFilter.IsActive)
        {
            parts.Add(scope.LabelFilter.Requirement("label", "labels"));
        }
